...
void timer_init(void)
{
  ...
  list_init(&sleepers); // Initialize the sleeping list
}
...
void timer_sleep(int64_t ticks)
{
  ...
  enum intr_level old_level = intr_disable();
  list_insert_ordered(&sleepers, &current_thread->elem, time_compare, NULL);
  current_thread->time_for_wakeup = current_time + ticks;
  thread_block();
  intr_set_level(old_level);
}
...
timer_interrupt(struct intr_frame *args UNUSED)
{
  ...
  for (e = list_begin(&sleepers); e != list_end(&sleepers); e = list_next(e))
  {
    ...
    if (t->time_for_wakeup <= timer_ticks())
    {
      list_remove(e);
      thread_unblock(t);
    }
