...
// FIXME:  Get the gclist for different types of objects
...
** mode, the threcad can still be modified before the end of the cycle,
...
static void stepgenfull(lua_State *L, global_State *g)
{
  ...
  {
#ifdef DEBUG
    printf("Return to generational mode\n");
#endif
    /* good collection? */
  ...
  {
#ifdef DEBUG
    printf("Stay in incremental mode\n");
#endif
...
static lu_mem atomic(lua_State *L)
{
  ...
  g->gcstate = GCSatomic;
  markobject(g, L); /* mark running thread */ // TODO: The running thread is marked
  /* registry and global metatables may be changed by API */
  markvalue(g, &g->l_registry);                      // TODO: The registry or global table is marked
  markmt(g); /* mark global metatables */            // TODO: The global metatables are marked
  work += propagateall(g); /* empties 'gray' list */ // TODO: The gray list is emptied
  /* remark occasional upvalues of (maybe) dead threads */
  work += remarkupvals(g);                         // TODO: Occasional upvalues of dead threads are remarked ( Need to look a bit more into this )
  work += propagateall(g); /* propagate changes */ // TODO: Changes are propagated
  g->gray = grayagain;
  work += propagateall(g); /* traverse 'grayagain' list */ // TODO: The grayagain list is traversed
  convergeephemerons(g);                                   // TODO: Ephemeron tables are converged ( Need to look a bit more into this )
  /* at this point, all strongly accessible objects are marked. */
  /* Clear values from weak tables, before checking finalizers */
  clearbyvalues(g, g->weak, NULL); // TODO: Values are cleared from weak tables
  clearbyvalues(g, g->allweak, NULL);
  origweak = g->weak;
  origall = g->allweak;
  separatetobefnz(g, 0); /* separate objects to be finalized */      // TODO: Objects to be finalized are separated
  work += markbeingfnz(g); /* mark objects that will be finalized */ // TODO: Objects that will be finalized are marked
  work += propagateall(g); /* remark, to propagate 'resurrection' */ // TODO: Resurrection is propagated
  convergeephemerons(g); /* finish ephemeron tables */               // TODO: Ephemeron tables are finished
  /* at this point, all resurrected objects are marked. */
  /* remove dead objects from weak tables */
  clearbykeys(g, g->ephemeron); /* clear keys from all ephemeron tables */ // TODO: Keys are cleared from all ephemeron tables
  clearbykeys(g, g->allweak); /* clear keys from all 'allweak' tables */   // TODO: Keys are cleared from all allweak tables
  /* clear values from resurrected weak tables */
  clearbyvalues(g, g->weak, origweak);                                 // TODO: Values are cleared from resurrected weak tables
  clearbyvalues(g, g->allweak, origall);                               // TODO: Values are cleared from resurrected allweak tables
  luaS_clearcache(g);                                                  // TODO: API string cache is cleared
  g->currentwhite = cast_byte(otherwhite(g)); /* flip current white */ // TODO: Change current white
...
// TODO: Note what happens in a single step
