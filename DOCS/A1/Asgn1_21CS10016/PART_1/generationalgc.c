...
/**
 * Run generational garbage collection and execute the 'test' function
 *
...
static void generationalgc(lua_State *L)
{
    ...
    // Use default parameters for the generational garbage collector
    if (lua_gc(L, LUA_GCGEN, LUAI_GENMINORMUL, LUAI_GENMAJORMUL) == -1)
    {
        fprintf(stderr, "Error calling lua_gc\n");
        exit(EXIT_FAILURE);
    }

    // change the parameters of the generational garbage collector
...
int main(int argc, char *argv[])
{
    ...
    // Run generational garbage collection and execute 'test'
    generationalgc(L);
