...
async function initContractFromOrg2Identity() {
    console.log(
        `${GREEN}--> Fabric client user & Gateway init: Using Org2 identity to Org2 Peer${RESET}`
      );
    ...
    try {
        ...
        console.error(`${RED}Error in connecting gateway using Org2 identity: ${error}${RESET}`);
        process.exit(1);
    }
}
...
`);
            ...
            try {
                await handleCommand(command, contractOrg2);
            } catch (error) {
