        ...
        try {
            listener = async (event) => {
                ...
                if (event.eventName === 'AddToMarket' && wishlist.has(asset.docID)) {
                    try {
                        const doc_bought = await contractOrg1.submitTransaction('BuyFromMarket', asset.docID);
            ...
            await contractOrg1.addContractListener(listener);
        } catch (eventError) {
...
`);
                ...
                try {
                    await handleCommand(command, contractOrg1, wishlist, already_in_the_market);
                } catch (error) {
 ...
 * @param {Set<string>} already_in_the_market The documents already in the market
...
/**
 * Function to print the details of the transaction records
 * 
 * @param {Array<Object>} records The list of transaction records
 * @returns {void}
 */
